...
import hahn_spors_estimation as hse
...
def run_exp(fig_folder):
    ...
    #dir_coeffs = shd.directivity_omni() * np.ones((pos_dyn.shape[0], 1))
    #estimates["proposed omni"] = mm.inf_dimensional_shd_dynamic(p_dyn, pos_dyn, arrays["eval"].pos, sequence, sim_info.samplerate, sim_info.c, reg_param_td, dir_coeffs)

    estimates["proposed"] = sfe.est_inf_dimensional_shd_dynamic(p_dyn, pos_dyn, arrays["eval"].pos, sequence, sim_info.samplerate, sim_info.c, reg_param_td)
    ...
    #print(f"MSE new version: {np.mean((p_eval - estimates['proposed omni'])**2)}")
    #print(f"MSE hardcoded omni version: {np.mean((p_eval - estimates['proposed'])**2)}")
    # dir_coeffs = shd.directivity_linear(0.5, mic_direction)
    # estimates["proposed cardioid"] = sfe.est_inf_dimensional_shd_dynamic(p_dyn_cardioid, pos_dyn_cardioid, arrays["eval"].pos, sequence[:seq_len], sim_info.samplerate, sim_info.c, reg_param_td, dir_coeffs)
