...
import aspcore.distance as dist_jax
...
def soundfield_estimation_comparison(
    ...
    )  # needs to be done before removing 0Hz bin
    error_per_sample(p_est_orig, p_true_orig, fig_folder, output_method=output_method)

    freqs_to_keep = np.logical_and(
        freqs > remove_freqs_below, freqs < remove_freqs_above
    )
    ...
    if points_for_errors is not None:
        p_est_masked = {
            name: sig[..., points_for_errors] for name, sig in p_est.items()
        }
    ...
    error_per_frequency(
        p_est_masked, p_true_masked, freqs, fig_folder, output_method=output_method
    )
    ...
    if num_ls > 1:
        for l in range(num_ls):
            ...
            p_est_single = {
                name: sig[:, l : l + 1, :] for name, sig in p_est_masked.items()
            }
    ...
    if shape == "rectangle":
        ...
        compare_soundfields_all_freq(
            ...
            output_method=output_method,
            num_examples=num_examples,
            points_for_errors=points_for_errors,
        )
        compare_soundfields_all_time(
            p_all_orig,
            p_est_orig,
            p_true_orig,
            freqs_orig,
        ...
        if points_for_errors is not None:
            raise NotImplementedError(
                "Excluding points from error is not implemented for circular shapes"
            )
        error_per_angle(
            pos_est, p_est, p_true, center, fig_folder, output_method=output_method
        )
        estimates_per_angle(
            pos_est, p_all, freqs, center, fig_folder, output_method=output_method
        )
...
def _cleanup_args(p_est, p_true, num_ls=1):
    ...
    for name, est in p_est.items():
        if (
            est.ndim == 3 and est.shape[-1] == 1
        ):  # assume we have vectors that have not been squeezed properly
...
def _get_freq_example_idxs(num_freqs, num_examples):
    if num_freqs <= num_examples:
        ...
        idxs = np.linspace(
            num_freqs / num_examples, num_freqs - num_freqs / num_examples, num_examples
        ).astype(int)
    return idxs
...
def freq_response(p_all, freqs, fig_folder, index_to_show, output_method="pdf"):
    ...
    utils.save_plot(output_method, fig_folder, "freq_response")


def time_response(
    p_all, fig_folder, index_to_show, output_method="pdf", scaling="linear"
):
    fig, axes = plt.subplots(len(index_to_show), 1, figsize=(8, 4 * len(index_to_show)))
...
def error_per_sample(p_est, p_true, fig_folder, output_method="pdf"):
    ...
    for name, est in p_est.items():
        ...
        error = np.mean(np.abs(true_td - time_est) ** 2, axis=(0, 1)) / np.mean(
            np.abs(true_td) ** 2
        )
...
def error_per_angle(pos_est, p_est, p_true, center, fig_folder, output_method="pdf"):
    ...
    fig, ax = plt.subplots(1, 1, figsize=(8, 4))
    for name, est in p_est.items():
        error_per_pos = np.mean(np.abs(est - p_true) ** 2, axis=(0, 1)) / np.mean(
            np.abs(p_true) ** 2, axis=(0, 1)
        )
...
def estimates_per_angle(
    ...
    freq_idxs = np.linspace(
        num_freqs / num_sf_plots, num_freqs - num_freqs / num_sf_plots, num_sf_plots
    )
    ...
    for f_idx, f in enumerate(freq_idxs):
        for i, (name, est) in enumerate(p_all.items()):
            axes[f_idx, 0].plot(
                sorted_angle, np.abs(est[f, 0, :][index_array]), label=name
            )
            axes[f_idx, 1].plot(
                sorted_angle, np.real(est[f, 0, :][index_array]), label=name
            )
            axes[f_idx, 2].plot(
                sorted_angle, np.imag(est[f, 0, :][index_array]), label=name
            )
    ...
    utils.save_plot(output_method, fig_folder, "estimates_per_angle")
...
def soundfield_image(
    sig, pos, pos_mic=None, title="", vminmax=None, ax=None, cmap="inferno"
):
    sig = np.squeeze(sig)
    if sig.ndim == 1:
        sig = sig[:, None]
...
def mse(p_est, p_true, fig_folder, num_ls=1):
    ...
    if num_ls > 1:
        mse = {}
        for name, p in p_est.items():
            mse[name] = np.mean(np.abs(p - p_true) ** 2, axis=(0, 2)) / np.mean(
                np.abs(p_true) ** 2, axis=(0, 2)
            )
        ...
        mse_db = {
            name: (10 * np.log10(mse_val)).tolist() for name, mse_val in mse.items()
        }
        with open(fig_folder.joinpath("mse_db_src.json"), "w") as f:
            json.dump(mse_db, f, indent=4)


def error_per_frequency(
    p_est, p_true, freqs, fig_folder, output_method="pdf", plot_name=""
):
    """Plots the normalized mean square error as a function of frequency"""
    fig, ax = plt.subplots(1, 1, figsize=(8, 4))
    for name, est in p_est.items():
        square_error = np.abs(est - p_true) ** 2
        error_per_freq = np.mean(square_error, axis=(1, 2)) / np.mean(
            np.abs(p_true) ** 2, axis=(1, 2)
        )
...
def compare_soundfields_all_freq(
    p_all,
    p_est,
    p_true,
    freqs,
    ...
    if num_ls > 1:
        if p_true is not None:
            compare_soundfields(
                {
                    name: 10 * np.log10(np.mean(np.abs(p_true - p) ** 2, axis=1))
                    for name, p in p_est.items()
                },
                freqs,
                pos_im,
                fig_folder,
                pos_mic=pos_mic,
                plot_name=f"{plot_name}_square_error",
                output_method=output_method,
                only_positive=True,
                num_examples=num_examples,
                points_to_set_colormap=points_for_errors,
            )
            compare_soundfields(
                {
                    name: 10
                    * np.log10(np.mean(np.abs(p_true - p) ** 2, axis=(0, 1)))[None, ...]
                    for name, p in p_est.items()
                },
                np.zeros((1)),
                pos_im,
    ...
    for l in range(num_ls):
        if num_ls > 1:
            ...
            p_true_l = p_true
            extra_name = ""
        _single_src_soundfield_comparison(
            p_all_l,
            p_est_l,
            p_true_l,
            freqs,
            ...
            output_method=output_method,
            num_examples=num_examples,
            plot_name=f"{plot_name}{extra_name}",
            points_to_set_colormap=points_for_errors,
        )
...
def _single_src_soundfield_comparison(
    p_all,
    p_est,
    p_true,
    freqs,
    ...
    num_examples=5,
    plot_name="",
    points_to_set_colormap=None,
):
    compare_soundfields(
        {name: np.abs(p) for name, p in p_all.items()},
        freqs,
        pos_im,
        fig_folder,
        pos_mic=pos_mic,
        plot_name=f"{plot_name}_abs",
        output_method=output_method,
        only_positive=True,
        num_examples=num_examples,
    )
    compare_soundfields(
        {name: np.real(p) for name, p in p_all.items()},
        freqs,
        pos_im,
        fig_folder,
        pos_mic=pos_mic,
        plot_name=f"{plot_name}_real",
        output_method=output_method,
        num_examples=num_examples,
    )
    compare_soundfields(
        {name: np.imag(p) for name, p in p_all.items()},
        freqs,
        pos_im,
        fig_folder,
        pos_mic=pos_mic,
        plot_name=f"{plot_name}_imag",
        output_method=output_method,
        num_examples=num_examples,
    )
    if p_true is not None:
        compare_soundfields(
            {name: 10 * np.log10(np.abs(p_true - p) ** 2) for name, p in p_est.items()},
            freqs,
            pos_im,
            fig_folder,
            pos_mic=pos_mic,
            ...
            only_positive=True,
            num_examples=num_examples,
            points_to_set_colormap=points_to_set_colormap,
        )
        compare_soundfields(
            {
                name: 10
                * np.log10(np.mean(np.abs(p_true - p) ** 2, axis=0, keepdims=True))
                for name, p in p_est.items()
            },
            np.zeros((1)),
            pos_im,
        ...
        compare_soundfields(
            {
                name: 10
                * np.log10(
                    np.mean(np.abs(p_true - p) ** 2, axis=0, keepdims=True)
                    / np.mean(np.abs(p_true) ** 2, axis=0, keepdims=True)
                )
                for name, p in p_est.items()
            },
            np.zeros((1)),
...
def compare_soundfields_all_time(
    p_all,
    p_est,
    p_true,
    freqs,
    ...
    if (
        freqs.shape[-1] > 10
    ):  # if there's a reasonable amount of frequencies, show time domain responses
        for l in range(num_ls):
            if num_ls > 1:
                p_all_l = {name: p[:, l, :] for name, p in p_all.items()}
                extra_name = f"_src_{l}"
            ...
            mean_energy = np.mean(
                np.stack(
                    [np.mean(np.abs(ir) ** 2, axis=0) for ir in ir_all.values()], axis=0
                ),
                axis=0,
            )
            ...
            compare_time_domain_soundfields(
                ir_all,
                pos_im,
                fig_folder,
                pos_mic=pos_mic,
                plot_name=f"timedomain{extra_name}",
                output_method=output_method,
                num_examples=num_examples,
                additional_time_indices=max_energy_idx,
            )

            if p_true is not None:
                ir_true = ft.irfft(p_true)
                mses = {
                    name: np.abs(ir_true - ir) ** 2
                    / np.mean(np.abs(ir_true) ** 2, axis=-1, keepdims=True)
                    for name, ir in ir_est.items()
                }
                ...
                compare_time_domain_soundfields(
                    ...
                    fig_folder,
                    pos_mic=pos_mic,
                    plot_name=f"timedomain_mse_db{extra_name}",
                    output_method=output_method,
                    num_examples=num_examples,
                    only_positive=True,
                    points_to_set_colormap=points_for_errors,
...
def compare_time_domain_soundfields(
    ir_all,
    pos_im,
    fig_folder,
    pos_mic=None,
    ...
    td_idxs = np.linspace(
        ...
        num_samples - num_samples / num_examples,
        num_examples,
    ).astype(int)
    ...
    for n in td_idxs:
        ...
        if points_to_set_colormap is not None:
            v_max = np.max(
                [np.max(ir[..., points_to_set_colormap, n]) for ir in ir_all.values()]
            )
            v_min = np.min(
                [np.min(ir[..., points_to_set_colormap, n]) for ir in ir_all.values()]
            )
...
def compare_soundfields(
    ...
    for f in freq_idxs:
        ...
        if points_to_set_colormap is not None:
            v_max = np.max(
                [np.max(sf[f, ..., points_to_set_colormap]) for sf in p_all.values()]
            )
            v_min = np.min(
                [np.min(sf[f, ..., points_to_set_colormap]) for sf in p_all.values()]
            )
        else:
            v_max = np.max([np.max(sf[f, ...]) for sf in p_all.values()])
            v_min = np.min([np.min(sf[f, ...]) for sf in p_all.values()])
...
def evaluate_freq_spatial_cov(
    estimates, cov_true, fig_path, freqs=None, plot_method="pdf", name_suffix=""
):
    ...
    mse = {
        name: np.mean(np.abs(est - cov_true) ** 2) / np.mean(np.abs(cov_true) ** 2)
    ...
    for est_name, est in estimates.items():
        mse = np.mean(np.abs(est - cov_true) ** 2, axis=(-1, -2)) / np.mean(
            np.abs(cov_true) ** 2, axis=(-1, -2)
        )
    ...
    utils.set_basic_plot_look(ax)
    utils.save_plot(
        plot_method, fig_path, f"spatial_cov_mse_db_no_normalization{name_suffix}"
    )

    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    est_true_normalized = (
        cov_true / np.trace(cov_true, axis1=-2, axis2=-1)[:, None, None]
    )
    for est_name, est in estimates.items():
        est_normalized = est / np.trace(est, axis1=-2, axis2=-1)[:, None, None]
        mse = np.sum(
            np.abs(est_normalized - est_true_normalized) ** 2, axis=(-1, -2)
        )  # / np.sum(np.abs(est_true_normalized)**2, axis=(-1, -2))
        mse_db = 10 * np.log10(mse)
        ax.plot(freqs, mse_db, label=est_name)
    ax.legend()
    ax.set_xlabel("Frequency")
    ...
    utils.set_basic_plot_look(ax)
    utils.save_plot(
        plot_method, fig_path, f"spatial_cov_mse_tracenormalized_db{name_suffix}"
    )

    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    est_true_normalized = cov_true / np.mean(
        np.abs(cov_true) ** 2, axis=(-1, -2), keepdims=True
    )
    for est_name, est in estimates.items():
    ...
    utils.set_basic_plot_look(ax)
    utils.save_plot(
        plot_method, fig_path, f"spatial_cov_mse_frobnormalized_db{name_suffix}"
    )

    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    mean_airm = {}
    for est_name, est in estimates.items():
        airm = jax.vmap(dist_jax.airm, in_axes=(0, 0))(est, cov_true)
    ...
    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    for est_name, est in estimates.items():
        frob_gevd = jax.vmap(dist_jax.frob_gevd_weighted, in_axes=(0, 0))(est, cov_true)
    ...
    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    for est_name, est in estimates.items():
        wishart_ll = jax.vmap(dist_jax.wishart_log_likelihood, in_axes=(0, 0, None))(
            est, cov_true, 5
        )
    ...
    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    for est_name, est in estimates.items():
        wasserstein = jax.vmap(dist_jax.wasserstein_distance, in_axes=(0, 0))(
            est, cov_true
        )
    ...
    for freq_example_idx in freq_example_idxs:
        ...
        fig, ax = plt.subplots(
            len(estimates_all), 3, figsize=(12, 4 * len(estimates_all))
        )
        ...
        utils.save_plot(
            plot_method,
            fig_path,
            f"spatial_cov_examples_freq_{freq_example}Hz{name_suffix}",
        )
