...
def inf_dimensional_shd_dynamic(
    p, pos, pos_eval, sequence, samplerate, c, reg_param, dir_coeffs=None, verbose=False
):
    """
    ...
    assert (
        seq_len % 2 == 0
    )  # Calculations later assume seq_len is even to get the Nyquist frequency
    ...
    if dir_coeffs is None:
        return _est_inf_dimensional_shd_omni(
            p, pos, pos_eval, sequence, samplerate, c, reg_param, verbose
        )
    if dir_coeffs.ndim == 2:
        dir_coeffs = dir_coeffs[
            None, :, :
        ]  # add a dimension for the number of frequencies
    ...
    est_sound_pressure = estimate_from_regressor(
        regressor, pos, pos_eval, wave_num[:num_real_freqs], dir_coeffs
    )
...
def _est_inf_dimensional_shd_omni(
    p, pos, pos_eval, sequence, samplerate, c, reg_param, verbose=False
):
    """Estimates the RIR at evaluation positions using data from a moving microphone
    using Bayesian inference of an infinite sequence of spherical harmonics
    ...
    dist_mat = (
        np.sqrt(np.sum((np.expand_dims(pos, 1) - np.expand_dims(pos, 0)) ** 2, axis=-1))
        / np.pi
    )
    ...
    psi += np.sinc(dist_mat * k[0]) * np.real_if_close(
        Phi[0, :, None] * Phi[0, None, :]
    )
    assert seq_len % 2 == 0  # following line is only correct if B is even
    psi += np.sinc(dist_mat * k[seq_len // 2]) * np.real_if_close(
        Phi[seq_len // 2, :, None] * Phi[seq_len // 2, None, :]
    )
    ...
    regressor = regressor[
        :num_real_freqs, :
    ]  # should be replaced by fixing the function calculating Phi instead

    # ======= Reconstruction of RIR =======
    est_sound_pressure = _estimate_from_regressor_omni(
        regressor, pos_eval, pos, k[:num_real_freqs]
    )
...
def est_spatial_spectrum_dynamic(
    p,
    pos,
    pos_eval,
    sequence,
    ...
    for f in range(num_real_freqs):  # freq_idx_list:
        ...
        Y_f = spspec.sph_harm_y(
            order[None, :], modes[None, :], angles[:, 1:2], angles[:, 0:1]
        )
...
def reconstruct_spatial_spectrum(params, pos_eval, wave_num, max_orders):
    ...
    for f in range(num_real_freqs):
        ...
        Y = spspec.sph_harm_y(
            order[None, :], modes[None, :], angles_eval[:, 1:2], angles_eval[:, 0:1]
        )
        rir_est[f, :] = np.sum(
            params[None, ord_idx : ord_idx + num_ord] * Y * j_num, axis=-1
        )  # / j_denom[None,:]
        ord_idx += num_ord
    return rir_est
...
def _seq_stft_bayesian_slow(sequence):
    ...
    for n in range(B):
        ...
        seq_vec = np.flip(
            seq_vec
        )  # so that we get n first and then n-i as we move later in the vector
        for f in range(B):
...
def estimate_from_regressor(regressor, pos, pos_eval, wave_num, dir_coeffs=None):
    ...
    for i in range(num_eval):
        kernel_val = shd.translated_inner_product(
            pos_eval[i : i + 1, :], pos, dir_omni, dir_coeffs, wave_num
        )
        est_sound_pressure[:, i : i + 1] = np.squeeze(
            kernel_val @ regressor[:, :, None], axis=-1
        )
    return est_sound_pressure
...
def _estimate_from_regressor_omni(regressor, pos_eval, pos, k):
    ...
    for f in range(num_real_freqs):
        kernel_val = ki.kernel_diffuse(pos_eval, pos, k[f : f + 1]).astype(complex)[
            0, :, :
        ]
...
def calculate_psi(pos, dir_coeffs, k, Phi, seq_len, num_real_freqs):
    ...
    for f in range(1, num_real_freqs - 1):
        ...
        psi_f = (
            np.squeeze(
                shd.translated_inner_product(
                    pos, pos, dir_coeffs, dir_coeffs, k[f : f + 1]
                ),
                axis=0,
            )
            * phi_rank1_matrix
    ...
    # since they will be real already for a real input sequence
    psi += np.squeeze(
        np.real(shd.translated_inner_product(pos, pos, dir_coeffs, dir_coeffs, k[0:1])),
        axis=0,
    ) * np.real_if_close(Phi[0, :, None] * Phi[0, None, :])
    psi += np.squeeze(
        np.real(
            shd.translated_inner_product(
                pos, pos, dir_coeffs, dir_coeffs, k[seq_len // 2 : seq_len // 2 + 1]
            )
        ),
        axis=0,
    ) * np.real_if_close(Phi[seq_len // 2, :, None] * Phi[seq_len // 2, None, :])
    return psi
...
def krr_moving_mic(
    ...
    for f in range(num_real_freqs):
        ...
        K += dft_weighting[f] * np.real(
            np.squeeze(kernel_func(pos, pos, wave_num[f : f + 1], *kernel_args))
            * phi_rank1_matrix
        )
    ...
    # ======= Reconstruction of RIR =======
    est_sound_pressure = reconstruct_from_krr_params(
        krr_params, pos_eval, pos, wave_num, kernel_func, kernel_args
    )
    if return_params:
...
def reconstruct_from_krr_params(
    krr_params, pos_eval, pos, wave_num, kernel_func=None, kernel_args=[]
):
    """Takes the regressor from inf_dimensional_shd_dynamic, and gives back a sound field estimate.
    Reconstructs the sound field at the evaluation points using the regressor matrix
    from est_inf_dimensional_shd_dynamic
    ...
    est_sound_pressure = np.zeros((num_real_freqs, pos_eval.shape[0]), dtype=complex)
    for f in range(num_real_freqs):
        kernel_val = kernel_func(
            pos_eval, pos, wave_num[f : f + 1], *kernel_args
        ).astype(complex)[0, :, :]
...
def krr_moving_mic_rff(
    ...
    basis_directions = mc.uniform_random_on_sphere(
        num_basis * num_real_freqs, rng
    ).reshape((num_real_freqs, num_basis, 3))
    ...
    params = ft.real_vec_to_dft_domain(
        params, scale=True
    )  # (num_real_freqs, num_basis)

    z_eval = np.stack(
        [
            pw.plane_wave(pos_eval, basis_directions[f, :, :], wave_num[f])
            for f in range(num_real_freqs)
        ],
        axis=0,
    ) / np.sqrt(num_basis)  # (num_real_freqs, num_eval, num_basis)
...
def _rff_z_matrix(pos, wave_num, Phi, basis_directions, num_basis, seq_len, N):
    ...
    Z = np.stack(
        [
            pw.plane_wave(pos, basis_directions[f, :, :], wave_num[f])
            for f in range(num_real_freqs)
        ],
        axis=0,
    ) / np.sqrt(num_basis)
...
def _get_real_basis_vector(pos, basis_directions, wave_num, phi_f):
    ...
    V_nd = ft.dft_domain_to_real_vec(
        V_nd, even=True, scale=True
    )  # (seq_len, num_basis)
